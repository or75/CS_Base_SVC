...
svc_msg_t* FindSvc()
{
	svc_msg_t* g_pReturn = nullptr;

	DWORD dwAddrSvcBad = CSX::Memory::FindString( HW_DLL , XorStr( "svc_bad" ) );
	DWORD dwAddrSvcNop = CSX::Memory::FindString( HW_DLL , XorStr( "svc_nop" ) );

	if ( dwAddrSvcBad && dwAddrSvcNop )
	{
		DWORD dwSvc_Sign[6] = { 0, dwAddrSvcBad, 0, 1, dwAddrSvcNop, 0 };	
		DWORD dwSvcMsg = CSX::Memory::FindPattern( HW_DLL , (char*)dwSvc_Sign , sizeof( dwSvc_Sign ) , 0 );

		if ( dwSvcMsg )
		{
			g_pReturn = (svc_msg_t*)dwSvcMsg;
		}
	}
